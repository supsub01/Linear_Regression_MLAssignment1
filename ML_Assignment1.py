...
def train_model(Phi, y):
  #
  #
    w=inv(Phi.transpose()@Phi)@Phi.transpose()@y
    return w
  #
  #
...
#From the given d values we tested, larger dimension values give a lesser validation/test error. 
#When d=24, the error is least.
#However, we cannot conclude that the error decreaces as d increaces because very large values of d may result in
#overfitting.
...
def radial_basis_transform(X, B, gamma=0.1):
  #
    RBK = np.zeros((X.shape[0],B.shape[0]))
    for i,x in enumerate(X):
        for j,y in enumerate(B):
            RBK[i,j] = np.exp(-gamma*np.linalg.norm(x-y)**2)
    return RBK   
  #
  #
...
def train_ridge_model(Phi, y, lam):
  ...
  #
    ...
    w=inv((Phi.transpose()@Phi)+(lam*I))@Phi.transpose()@y
    return w
  #
  #
...
Range=[10**-3, 10**-2, 10**-1, 1, 10, 10**1, 10**2, 10**3]

w = {}               # Dictionary to store all the trained models
validationErr = {}   # Validation error of the models
testErr = {}         # Test error of all the models

for lam in Range:  # Iterate over polynomial degree
    #print(lam)
...
#From the given lamba values we tested, smaller lambda values give a lesser validation/test error. 
#When lambda=0.001, the error is least.
#However, we cannot conclude that the error decreaces as lambda decreaces because very small values of lambda may result in
#overfitting. 
...
plt.legend(['true'] + list(Range))
plt.axis([-8, 8, -15, 15])

#As lambda increases, the model becomes more linear. 
